...
def continent():
    ...
    error = 0
    success = 0
    x = pan.read_excel('国家收入分组.xlsx' ) #encoding="gbk"
    oricountry = pan.read_excel('各国每年移民数量处理/country_count/90.xlsx', encoding="gbk") 
    txt = open("./income.txt",mode='w',encoding='UTF-8')
    # print(oricountry.iloc[2,3])
    # print(x.iloc[57,3])
    for i in range(oricountry.index.size):
      for p in range(x.index.size):
        for q in range(3): # 6大洲
          if(oricountry.iloc[i,2] == x.iloc[p,q]):
            continentoutcount[q] = continentoutcount[q] + oricountry.iloc[i,3]
